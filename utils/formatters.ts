...
export const formatEmailPix = (v: string) => {
  ...
  // Busca a posição do .com
  const comIndex = email.indexOf('.com');
  if (comIndex !== -1) {
    // Se encontrou .com, corta a string exatamente no final do ".com"
...
export const formatPixKey = (value: string, type: string) => {
  switch (type) {
    ...
    case 'ALEATORIA':
      // Mantém apenas letras e números, removendo espaços e caracteres especiais
...
/**
 * Gera a URL do QR Code NFC-e conforme padrão solicitado:
 * https://www.sefaz.{UF}.gov.br/nfce/consulta?p={CHAVE_ACESSO}|{VERSAO}|{AMBIENTE}|{CSC}
 */
