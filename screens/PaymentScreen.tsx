...
const PaymentScreen: React.FC<{ onConfirm: () => void }> = ({ onConfirm }) => {
  ...
  const totalValue = useMemo(() => {
    ...
    return fuels.reduce((acc, fuel) => {
      const q = parseLocaleNumber(fuel.quantity);
      let p = parseLocaleNumber((isCard && fuel.unitPriceCard && parseLocaleNumber(fuel.unitPriceCard) > 0) ? fuel.unitPriceCard : fuel.unitPrice);
      return acc + (q * p);
    }, 0);
  ...
  const pixPayload = useMemo(() => {
    ...
    return '';
  }, [invoiceData.formaPagamento, postoData.chavePix, postoData.razaoSocial, totalValue, postoData.tipoChavePix]);
  ...
  return (
    <div className="space-y-8 animate-reveal">
      <div className="flex flex-col items-center">
         <span className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em] mb-3">Valor para Checkout</span>
        ...
        {methods.map(m => {
          ...
          return (
            <button 
              ...
              className={`flex flex-col items-center justify-center p-6 rounded-[2rem] border-2 transition-all duration-300 ${active ? 'border-indigo-500 bg-indigo-500/10 text-white shadow-lg shadow-indigo-500/20' : 'border-white/5 bg-white/5 text-slate-500'}`}
            >
      ...
      <div className="glass-card rounded-[2.5rem] p-8 flex flex-col items-center space-y-6 relative overflow-hidden min-h-[350px] justify-center">
        {paymentSuccess && (
          <div className="absolute inset-0 btn-primary z-50 flex flex-col items-center justify-center text-white animate-reveal">
            ...
            <p className="text-xs font-bold opacity-80 uppercase tracking-widest mt-2">Redirecionando...</p>
          </div>
        )}
              ...
              <div className="p-4 bg-white rounded-3xl shadow-2xl relative group">
                <div className={`transition-all duration-500 ${verifyingPayment ? 'opacity-20 blur-sm' : ''}`}>
                   {qrCodeUrl ? (
                     ...
                     <div className="w-48 h-48 flex flex-col items-center justify-center text-slate-300 border-2 border-dashed border-slate-200 rounded-xl">
                        <QrIcon size={40} className="mb-2 opacity-20" />
                        <span className="text-[8px] font-black uppercase text-center px-4">Configure a chave Pix nos Dados</span>
                     </div>
                   )}
                </div>
                {verifyingPayment && (
              ...
              <div className="text-center w-full">
                 ...
                 {pixPayload && (
                   <button 
                    ...
                    className="flex items-center gap-2 mx-auto px-6 py-3 bg-emerald-500/10 text-emerald-500 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-emerald-500/20 transition-all border border-emerald-500/20"
                   >
      ...
      {!paymentSuccess && (
        <button 
          ...
          className="w-full btn-primary py-5 rounded-[2rem] text-white font-black text-sm shadow-2xl flex items-center justify-center gap-3 tracking-[0.1em] disabled:opacity-50"
        >
