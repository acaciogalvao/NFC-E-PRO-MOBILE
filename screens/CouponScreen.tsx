...
const CouponScreen: React.FC = () => {
  ...
  if (!invoiceData || !postoData) {
    return <div className="p-10 text-center text-slate-500 font-bold uppercase text-xs">Carregando cupom...</div>;
  }
  ...
  const paymentMethodLabel = getPaymentLabel(invoiceData.formaPagamento || 'DINHEIRO');
  ...
  const fullData = { posto: postoData, invoice: invoiceData, fuels: fuels || [], calculations: calcData };
    ...
    <div className="flex flex-col items-center min-h-full pb-10 bg-slate-100/50 dark:bg-transparent">
      ...
      <div className="w-fit bg-white p-3 rounded-xl shadow-lg border border-slate-200 mb-4 flex justify-center print:bg-transparent print:p-0 print:border-none print:shadow-none print:w-full print:max-w-none print:block transition-all animate-fade-in">
        <div id="printable-receipt" className="relative box-border transition-all">
             {currentLayout && currentLayout.id === 'modelo_guimaraes' && <GuimaraesReceipt data={fullData as any} layout={currentLayout} width={paperWidth} />}
             {currentLayout && currentLayout.id === 'modelo_almeida' && <AlmeidaReceipt data={fullData as any} layout={currentLayout} width={paperWidth} />}
             {(!currentLayout || (currentLayout.id !== 'modelo_guimaraes' && currentLayout.id !== 'modelo_almeida')) && <StandardReceipt data={fullData as any} layout={currentLayout} width={paperWidth} />}
        </div>
      </div>
    </div>
  );
